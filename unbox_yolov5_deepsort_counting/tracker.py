...
def draw_bboxes(image, bboxes, line_thickness):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        # o_y1 = int(y1*1080/540)
        # o_y2 = int(y2*1080/540)
        # o_x1 = int(x1*1920/960)
        # o_x2 = int(x2*1920/960)


        c1, c2 = (x1, y1), (x2, y2)
        #if str(cls_id)=='person' and str(pos_id)=='11':
        #检测框
        cv2.rectangle(image, c1,c2, (0, 255, 255), thickness=line_thickness, lineType=cv2.LINE_AA)
        #cv2.rectangle(image, c1, c2, (0, 255, 255), thickness=line_thickness, lineType=cv2.LINE_AA)

        font_thickness = max(line_thickness - 1, 1)
        t_size = cv2.getTextSize(cls_id, 0, fontScale=line_thickness / 3, thickness=font_thickness)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(image, c1, c2, color, -1, cv2.LINE_AA)  # filled 检测框抬头
        cv2.putText(image, '{} ID-{}'.format(cls_id, pos_id), (c1[0], c1[1] - 2), 0, line_thickness / 3,
                    [225, 0, 0], thickness=font_thickness, lineType=cv2.LINE_AA)
        #print(pos_id)
